...
try:
    burst_sound = pygame.mixer.Sound(os.path.join(BASE_FOLDER, "burst.mp3"))
except:
    print("⚠ burst.wav not found")

try:
    error_sound = pygame.mixer.Sound(os.path.join(BASE_FOLDER, "error.mp3"))
except:
    print("⚠ error.wav not found")
...
def main():
    ...
    while running:
        ...
        # UI
        screen.blit(
            font_small.render(f"Score: {score}", True, (0, 0, 0)),
            (20, 20)
        )
        screen.blit(
            font_small.render("Press 0–9 (Top / NumPad)", True, (0, 0, 0)),
            (20, HEIGHT - 40)
        )
